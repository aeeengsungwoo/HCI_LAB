...
def process_image_with_all_thresholds(image_path, threshold_value=150, max_value=150, grayscale_output_path=None,
                                      binary_output_path=None, otsu_output_path=None, adaptive_output_path=None):
    ...
    adaptive_img = cv2.adaptiveThreshold(gray_img, max_value, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, 2)

    plt.figure(figsize=(5, 5))
    plt.imshow(adaptive_img, cmap='gray')
    plt.title(f'Lenna (Adaptive Thresholded, MaxValue={max_value})')
    plt.axis('off')
    plt.show()

    if adaptive_output_path:
        cv2.imwrite(adaptive_output_path, adaptive_img)
        print(f"Adaptive thresholded image saved as {adaptive_output_path}")


try:
    process_image_with_all_thresholds(
        'lenna.png',
        threshold_value=150,
        max_value=150,
        ...
        adaptive_output_path='lenna_adaptive.png'
    )
